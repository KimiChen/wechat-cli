...
@click.option("--limit", default=20, help="返回数量（最大 500）")
...
def search(ctx, keyword, chat, start_time, end_time, limit, offset, fmt, msg_type):
    ...
    if len(chat_names) == 1:
        ...
        resolved, unresolved, missing = resolve_chat_contexts(
            chat_names, app.msg_db_keys, app.cache, app.decrypted_dir
        )
        if not resolved:
            details = []
            if unresolved:
                details.append("未找到: " + "、".join(unresolved))
            if missing:
                details.append("无消息记录: " + "、".join(missing))
            message = "错误: 没有可查询的聊天对象"
            if details:
                message += "\n" + "\n".join(details)
        ...
        entries = []
        failures = []
        for resolved_ctx in resolved:
            found_entries, found_failures = collect_chat_search(
                resolved_ctx,
                names,
                keyword,
                app.display_name_fn,
        ...
        if missing:
            failures.append("无消息记录: " + "、".join(missing))
